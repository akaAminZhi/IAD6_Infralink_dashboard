...
def test_normalize_equipment_marks_battery_as_not_test_tracked(tmp_path) -> None:
    workbook_path = tmp_path / "SystemElements_battery.xlsx"
    row = [None] * len(SOURCE_COLUMNS)
    row[SOURCE_COLUMNS.index("Unique ID")] = "IAD06-INV6-H1-1 BATTERY 2"
    make_workbook(workbook_path, rows=[row])

    records = normalize_equipment(str(workbook_path))

    assert records[0]["test_tracking_required"] is False
